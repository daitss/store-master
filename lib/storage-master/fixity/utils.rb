...
module FixityUtils
  ...
  def FixityUtils.parse_options args
    ...
    opts = OptionParser.new do |opts|
      ...
      opts.on("--server-address HOSTNAME[:PORT]",  String, "The address of the storage-master web service - port defaults to 80") do |host_name|
      ...
      opts.on("--db-storage-master-key KEY", String, "The key for the store master database in the database yaml configuration file")  do |key|
    ...
    raise "No storage-master database key to the DB configuration file (#{conf.db_config_file}) provided"  unless conf.db_store_master_key
