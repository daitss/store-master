...
# TODO: remove StoreMasterModel:: and do an include, once through refactoring it out of StoreMaster
...
def postgres_setup
  ...
  StoreMasterModel.setup_db(File.join(HOME, 'db.yml'), 'store_master_postgres')
  StoreMasterModel.create_tables
end

def mysql_setup
  # DataMapper::Logger.new(STDERR, :debug)
  StoreMasterModel.setup_db(File.join(HOME, 'db.yml'), 'store_master_mysql')
  StoreMasterModel.create_tables
end
...
share_examples_for "DataMapper Package class with any DB, when it" do
  ...
  it "should let us create pools with differing read preferences, and order them " do
    ...
    pool1 = StoreMasterModel::Pool.add(pool('a'), 10)
    pool2 = StoreMasterModel::Pool.add(pool('b'))
  ...
  it "should not let us create pools with the same location" do
    ...
    pool1 = StoreMasterModel::Pool.add(pool('c'))
    pool1.save.should == true

    lambda { StoreMasterModel::Pool.add(pool('c')) }.should raise_error
  end


  it "should allow us to get a posting URL for a pool" do
    pending "This test now requires an active pool"
    pool = StoreMasterModel::Pool.add(pool('poster'))
  ...
  it "should let us retrieve an associated pool from a copy" do
    ...
    pkg1  = StoreMasterModel::Package.create(:ieid => IEID, :name => name(4))


    copy2time = DateTime.now - 100
    copy1 = StoreMasterModel::Copy.create(:store_location => foo, :pool => pool1)
    copy2 = StoreMasterModel::Copy.create(:store_location => bar, :pool => pool2, :datetime => copy2time)
    ...
    pkg2 = StoreMasterModel::Package.first(:name => name(4))
  ...
  it "should let us mark a package as deleted" do
    ...
    pkg  = StoreMasterModel::Package.lookup(name(101))
    pkg.should_not == nil
  ...
  it "should let us associate copies URL with a packge, setting the time or defaulting it, storing and retreiving it" do
    ...
    pkg1  = StoreMasterModel::Package.first(:name => name(1))
    ...
    pkg2 = StoreMasterModel::Package.first(:name => name(1))
  ...
  it "should not let us create copies within the same pool for a given package" do
    ...
    pkg  = StoreMasterModel::Package.create(:ieid => IEID, :name => name(3)) 
    pool = StoreMasterModel::Pool.first(:services_location => pool('d'))

    baz  = 'http://bar.example.com/baz'
    quux = 'http://bar.example.com/quxx'

    copy1 = StoreMasterModel::Copy.create(:store_location => quux, :pool => pool)
    copy2 = StoreMasterModel::Copy.create(:store_location => baz,  :pool => pool)
  ...
  it "should provide a uri method for a copy record that includes basic authentication information from the pool, but doesn't print" do
    ...
    copy = StoreMasterModel::Copy.create(:store_location => 'http://example.com/', :pool => pool)
  ...
  it "should allow us to create reserved names based on an IEID" do
    ...
    res1 = StoreMasterModel::Reservation.create(:ieid => IEID, :name => name(1))
    res1.saved?.should == true
    res1.name.should == name(1)

    res2 = StoreMasterModel::Reservation.create(:ieid => IEID, :name => name(2))
    res2.saved?.should == true
    res2.name.should == name(2)
  end

  it "should not allow us to reuse reserved names" do
    res = StoreMasterModel::Reservation.create(:ieid => IEID, :name => name(1))
